...
class TestFrequency:
    ...
    @freezegun.freeze_time(start_date.replace(hour=0, minute=0))
    async def test_device_responds_zero(self):
        frequency = Frequency()

        frequency.update_state(struct.pack(">BIHB", 0, 0, 0, 0))

        assert frequency.next_run_time is None
        assert frequency.duration_minutes == 0
        assert frequency.interval_hours == 0
    ...
    async def test_frequency_currently_running(self):
        ...
        frequency.duration_minutes = 10
        frequency.interval_hours = 6
        frequency.start_time = start_date.replace(hour=0, minute=0).time()
